...
The backend provides `task.readonly: boolean` on every task. When `true`, dhtmlx natively prevents the task from being dragged or resized. The gantt-renderer has no equivalent — there's no way to lock an individual task against drag.

Without this, consumers must check `task.readonly` in `onTaskMove`/`onTaskResize` callbacks and return `false` to abort — but the bar still moves visually during drag, then snaps back. The drag should be **prevented before it starts**, not reverted after.
...
- `cleanupDrag` may be undefined when readonly — the registry type already uses optional fields (`cleanupLinkHandles?:`, `cleanupProgressDrag?:`), but `cleanupDrag` is required. Make it optional.
...
The backend provides `link.readonly: boolean` on every link. In dhtmlx, `readonly: true` prevents the link from being deleted via UI. Since the current app has `drag_links: false` and no link-delete UI, this field has no observable effect today — but the backend sends it, so it must be preserved through data loading.

This is a purely additive schema change with no behavioral impact.
...
No behavioral changes needed. The field passes through `parseGanttInput` and is preserved in `link.data` or as a dedicated field.
...
The backend provides `link.lag: number` on every link (in days, can be negative). In dhtmlx, `lag` adds a delay (positive) or overlap (negative) between dependent tasks, visualized as a gap or overlap in the dependency arrow. Current app uses `work_time: false, duration_unit: 'day'`, so lag is in calendar days.

Without lag, dependency arrows connect directly from anchor to anchor with no offset — losing the visual gap that users expect when tasks have lead/lag time.
...
a. The `PixelMapper` already has `durationToWidth(hours)` → px. Pass a pixel conversion to `routeLinks`.
...
c. In the `route` function (or `getAnchors`), apply lag as a rightward x-offset on the source anchor:
...
The simplest approach: apply `lagPx` as a positive rightward offset to `sx` in `getAnchors`. This creates a visual gap between the predecessor bar edge and the arrow exit point.
...
- `pnpm run ci` passes
- `pnpm run integration-test` passes
- Add a unit test for `routeLinks` with lag: a link with positive lag should have its first anchor point shifted right
...
The dhtmlx app uses `gantt.templates.task_class` to apply CSS classes to task bars, enabling 4 distinct visual styles: Group (green), Locked (orange), Decision (red), and Inactive (grey). The class is applied to the bar DOM element. The priority chain is: isGroup → isInactive → isDecision → isLocked → default.

The gantt-renderer currently supports `task.color` for bar fill color only, which is insufficient — the app needs different text colors, border colors, and milestone styling per task type, not just bar fills.
...
- Add an integration test: task with a custom CSS class renders with the correct visual (e.g., different color)
...
The dhtmlx app uses `gantt.templates.tooltip_text` to render an HTML tooltip on bar hover showing: task type title, task name, status, dates (with/without time depending on locked), duration, and progress. The tooltip is styled via `.gantt_tooltip` CSS.

The gantt-renderer has no tooltip system — no way to show contextual information on hover.
...
Add the internal binding in the constructor (around line 175, in the `#cbs` object). The internal callback format:
