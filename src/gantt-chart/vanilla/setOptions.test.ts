...
describe('setOptions', () => {
	...
	it('updates callbacks without re-rendering DOM', () => {
		...
		const newSelect = vi.fn<(_id: number | null) => void>();
		instance.setOptions({onSelect: newSelect});

		expect(container.innerHTML).toBe(initialHtml);

		instance.select(1);
		expect(newSelect).toHaveBeenCalledWith(1);
	});
	...
	it('updates onMove callback and uses it after setOptions', () => {
		const container = createContainer();
		const instance = mountTracked(container, INPUT);

		const moveSpy = vi.fn<(_payload: {id: number; startDate: Date}) => void>();
		instance.setOptions({onMove: moveSpy});

		expect(container.querySelector('.gantt-root')).not.toBeNull();
	});

	it('updates onResize callback and uses it after setOptions', () => {
		const container = createContainer();
		const instance = mountTracked(container, INPUT);

		const resizeSpy = vi.fn<(_payload: {id: number; durationHours: number}) => void>();
		instance.setOptions({onResize: resizeSpy});
	...
	it('updates onLinkCreate callback and uses it after setOptions', () => {
		...
		const linkSpy = vi.fn<(_payload: {sourceTaskId: number; targetTaskId: number; type: 'FS'}) => void>();
	...
	it('does nothing when only callbacks are changed', () => {
		...
		instance.setOptions({
			onSelect: vi.fn<(_id: number | null) => void>(),
			onMove: vi.fn<(_payload: {id: number; startDate: Date}) => void>(),
			onTaskDoubleClick: vi.fn<(_payload: {id: number; source: 'grid' | 'bar' | 'milestone'}) => void>(),
			onTaskEditIntent: vi.fn<(_payload: {id: number; source: 'grid' | 'bar' | 'milestone'; trigger: 'doubleClick'; task: unknown}) => void>(),
			onAdd: vi.fn<(_payload: {parentId: number}) => void>(),
		});
